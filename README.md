...
- Create environment variable USERAREA pointing to your `etc/griblists` (Default is `$PERM/extractGrib/$EXP/etc/griblists`)
...
- These should be saved as some `etc/paramlists/cccc/<param>.list` or `etc/paramlists/<param>.list`

### Parameter namelists

- Parameter namelist fragments are stored in `etc/namelist_inc/cccc/30-<paramlist>_P-b.inc` or `etc/namelist_inc/30-<paramlist>_P-b.inc`
...
- Subdomain namelist fragments are stored in `etc/namelist_inc/cccc/20-<proj>.inc` or `etc/namelist_inc/20-<proj>.inc`
...
- Griblists are stored in `etc/griblists/cccc/control.list`, `etc/griblists/cccc/standing.list` and `etc/griblists/cccc/user.list`
...
This output should be saved to `etc/namelist_inc/cccc/20-<proj>.inc` or `etc/namelist_inc/20-<proj>.inc` where 'proj' is the name of the projection to be used in a [griblist](#griblists)
...
Typical usage is:
	...
	bin/do_paramset.sh -b -o etc/namelist_inc/cccc/ etc/paramlists/cccc/<param>.list
	bin/do_paramset.sh -b -o etc/namelist_inc/ etc/paramlists/<param>.list
...
2. The [parameter](#parameter-lists) file `etc/namelist_inc/knmi/30-en_B-p.inc` is used to to generate a `pppkey%...` namelist to be stored in memory 
	...
	If the file `etc/namelist_inc/knmi/30-en_B-p.inc` does not exist the file `etc/namelist_inc/30-en_B-p.inc` will be used if it exists. 
